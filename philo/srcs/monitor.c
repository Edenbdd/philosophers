...
/*   Updated: 2025/02/05 13:21:32 by aubertra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_meals_eaten(t_data *monitor)
{
	...
	count = 0;
	max_reached = 0;
	while (monitor->max_nb_of_meals != -2 && count < monitor->number_of_philo)
	{
...
int	check_death(t_data *monitor)
{
	...
	while (count < monitor->number_of_philo)
	{
		...
		pthread_mutex_lock(curr_philo->m_eating);
		if ((get_exact_time()
				- curr_philo->timestamp_last_meal) == monitor->time_to_die)
		{
			pthread_mutex_unlock(curr_philo->m_eating);
